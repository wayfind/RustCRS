...
test.describe('API Stats - 交互功能', () => {
  ...
  test('应该有刷新按钮或自动刷新功能', async ({ page }) => {
    // 查找刷新按钮
    const refreshButton = page.getByRole('button', { name: /刷新|refresh|reload/i })
      .or(page.locator('button').filter({ has: page.locator('[class*="refresh"], [class*="reload"]') }));
  ...
  test('应该支持数据筛选（如果有）', async ({ page }) => {
    ...
    const filterElements = page.locator(
      'select, .filter, .dropdown, .el-select, [class*="filter"]'
    );
...
test.describe('API Stats - 空状态处理', () => {
  test('应该优雅处理无数据情况', async ({ page }) => {
    ...
    const emptyState = page.locator(
      '.empty, .no-data, .el-empty, [class*="empty"]'
    );
