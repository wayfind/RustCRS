...
.PHONY: help install setup dev start test lint clean docker-up docker-down service-start service-stop service-status logs cli-admin cli-keys cli-accounts cli-status rust-dev rust-run rust-release rust-build rust-test init-redis start-all stop-all verify
...
help:
	...
	@echo ""
	@echo "  🦀 Rust 版本（推荐）："
	@echo "    rust-dev       - Rust 开发模式启动全部服务"
	@echo "    rust-run       - 同 rust-dev"
	@echo "    rust-release   - Rust 发布模式启动"
	@echo "    rust-build     - 构建 Rust 发布版本"
	@echo "    rust-test      - 运行 Rust 测试"
	@echo "    init-redis     - 启动并初始化 Redis"
	@echo "    start-all      - 启动全部服务（Redis + 后端 + 前端）"
...
# Rust 统一启动（Redis + 后端 + 前端）
start-all:
	@echo "🚀 启动全部服务（Redis + 后端 + 前端）..."
	bash scripts/start-all.sh dev

stop-all:
	@echo "🛑 停止所有服务..."
	@pkill -f "claude-relay" || echo "后端未运行"
	@docker stop redis-dev 2>/dev/null || echo "Redis 未运行"
	@pkill -f "vite" || echo "前端未运行"
	@echo "✅ 所有服务已停止"

# Rust 开发模式（完整环境）
rust-dev:
	@echo "🦀 Rust 开发模式启动（完整环境）..."
	bash rust-dev.sh
...
# Rust 完整检查（格式+检查+测试）
rust-check: rust-fmt-check rust-lint rust-test-unit
	@echo "✅ Rust 代码检查完成！"

# Rust 快速启动组合
rust-quick: verify init-redis rust-backend
	@echo "🎉 Rust 后端已启动！"
	@echo "💡 提示: 运行 'make rust-frontend' 启动前端"

# Rust 完整开发环境
rust-full: verify init-redis start-all
	@echo "🎉 Rust 完整开发环境已启动！"